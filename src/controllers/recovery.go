...
func (ctrl *RecoveryController) GetDeadLetterMessages(c *gin.Context) {
	...
	middleware.RespondSuccess(c, http.StatusOK, messages)
}
...
func (ctrl *RecoveryController) RetryDeadLetterMessage(c *gin.Context) {
	...
	if err != nil {
		middleware.RespondNotFound(c, "Message not found or access denied")
		return
	}

	middleware.RespondSuccess(c, http.StatusOK, gin.H{"message": "Dead letter message retry initiated"})
}
...
func (ctrl *RecoveryController) RestartWorkflow(c *gin.Context) {
	...
	if execution.Status != "error" && execution.Status != "partially_failed" {
		middleware.RespondBadRequest(c, "Only failed or partially failed executions can be restarted")
		return
	}
...
func (ctrl *RecoveryController) ReExecuteNode(c *gin.Context) {
	...
	if nodeExecution == nil {
		middleware.RespondNotFound(c, "Node execution not found")
		return
	}

	// Check if node is in a failed state
	if nodeExecution.Status != "error" {
		middleware.RespondBadRequest(c, "Only failed nodes can be retried")
		return
	}
	...
	if !canRetry {
		middleware.RespondBadRequest(c, "This node type cannot be retried individually")
		return
	}
	...
	if nodeExecution.Input != nil {
		if err := json.Unmarshal([]byte(*nodeExecution.Input), &nodeInput); err != nil {
			middleware.RespondInternalError(c, "Failed to parse node input")
			return
		}
	} else {
	...
	middleware.RespondSuccess(c, http.StatusOK, gin.H{"message": "Node retry initiated"})
}
